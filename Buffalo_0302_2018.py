...
file = '2A.GPM.DPR.V8-20180723.20180302-S035023-E052255.022768.V06A.HDF5'
...
ind1 = np.where((lon[:,0]>=-81)) #Where are the DPR lons >= -100
ind2 = np.where((lon[:,0])<=-75) #Where are lons <= -85
ind3 = np.intersect1d(ind1,ind2) #Both conditions need to be satisfied here

###Define cross-section length for ka band


ind1_ka = np.where((lon_ka[:,0]>=-81))
ind2_ka = np.where((lon_ka[:,0]<=-75))
...
echo_top_index = np.array([15,34,34,27,16,17,17,20,28,34,38,39,44,40,36,47,37,29,34,26,36,37,41,39,42,41,40,37,40,37,39,40,36,37,36,34,33,35,37,34,33,32,33,35,33,33,35,37,28,27,31,32,35,31,31,28,30,40,39,43,40,42,37,32,27,26,33,30,28,31,29,34,28,29,26,23,27,])
...
plt.title('3/2/2018 Along-Track Echo Top Height', fontsize = 18)
...
plt.label(r'$\overline x = 3.69$ km')
plt.label(r'$S = 0.33$ km')
plt.legend()
...
plt.title('Buffalo, NY 3/2/2018 Along-Track KuPR', size = title_size)
...
plt.title(r'GPM Overpass 3/2/2018 $D_{M}$ ', size = 20)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 3/2/2018 $log_{10 }(N_{W})$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(altitude[0],altitude[1])
cbar = plt.colorbar()
cbar.ax.tick_params(labelsize = label_size)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 3/2/2018 KuPR ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(altitude[0],altitude[1])
cbar = plt.colorbar()
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 3/2/2018 KaPR ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(altitude[0],altitude[1])
cbar = plt.colorbar()
...
xlim = np.array([-82,-75]); ylim = np.array([40,45])
...
plt.title('GPM Overpass 3/2/2018 KuPR', size = 20)

label = 'KBUF'
x2star,y2star = m(-78.719,42.941)
m.plot(x2star,y2star,'ro',markersize=12, color = 'k')
plt.text(x2star-0.4,y2star+0.15,label, weight = 'bold', size = 10)


latitude_path = [42.15,42.25,42.7,43.,43.5,43.75,43.8,43.6,43.3,42.95,42.65,42.3,42.05,42.15]
longitude_path = [-79.05,-79.5,-79.85,-79.9,-79.6,-79.4,-78.75,-77.95,-77.65,-77.55,-77.6,-77.85,-78.6,-79.05]
...
plt.title('GPM Overpass 3/2/2018 Near Surface Precipitation Rate', size = 20)
...
plt.title('GPM Overpass 3/2/2018 LWP/IWP Ratio', size = 20)
...
plt.title('GPM Overpass 3/2/2018 IWP', size = 20)
...
plt.title('GPM Overpass 3/2/2018 Precipitation Category', size = 20)
...
plt.title('GPM Overpass 3/2/2018 Snowfall Rate (KuPR)', size = 20)
...
plt.title('GPM Overpass 3/2/2018 Snowfall Rate (KaPR)', size = 20)
