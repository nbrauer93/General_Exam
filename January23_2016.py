...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 13 16:30:53 2020
...
file = '2A.GPM.DPR.V8-20180723.20160123-S120355-E133625.010811.V06A.HDF5'
...
ind1 = np.where((lon[:,0]>=-72.5)) #Where are the DPR lons >= -100
ind2 = np.where((lon[:,0])<=-70) #Where are lons <= -85
...
plt.title('Long Island 1/23/2016 Along-Track KuPR', size = title_size)
...
echo_top_array_index = np.array([39,44,43,42,40,42,42,38,41,28,29,37,28,39,34,33,27,25,27,27,25,29,28,33,33,39,53,55,57,56,57,43,48,52,51,47,47,54,52,47,48,51,48,45,45,44,44,45,46,45,46,45,46,50,52,48,45,50,39,39,37,26,23,26,32,26,28,33,32,30,26])
echo_top_height = echo_top_array_index*0.124932  

echo_top_mean = np.nanmean(echo_top_height)
echo_top_std = np.nanstd(echo_top_height)

bins = np.arange(1,8,step = 0.5)
plt.hist(echo_top_height,bins = bins,edgecolor='black', linewidth=1.2, color = 'green')
plt.xlabel('Echo Top Height (km)', fontsize = 20)
plt.ylabel('Frequency', fontsize = 20)
plt.title('1/23/2016 Along-Track Echo Top Height', fontsize = 18)
...
plt.title(r'GPM Overpass 1/23/2016 $D_{M}$ ', size = 20)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 1/23/2016 $log_{10 }(N_{W})$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(altitude[0],altitude[1])
cbar = plt.colorbar()
cbar.ax.tick_params(labelsize = label_size)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 1/23/2016 KuPR ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(altitude[0],altitude[1])
cbar = plt.colorbar()
...
cmin =12.; cmax = 60.; cint = 2.5; clevs = np.round(np.arange(cmin,cmax,cint),2)
nlevs = len(clevs) - 1; cmap = plt.get_cmap(name='pyart_NWSRef',lut=nlevs)
pm = plt.pcolormesh(R_gpm/1000., y, ka, cmap='pyart_NWSRef',vmin=vmin,vmax=vmax)
pm2 = plt.plot(R_gpm/1000., zero_deg_isotherm, '--', color = 'k')
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 3/2 0350 UTC KaPR ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(altitude[0],altitude[1])
cbar = plt.colorbar()
...
xlim = np.array([-76,-70]); ylim = np.array([38,43])
...
plt.title('GPM Overpass 1/23/2016 KuPR', size = 20)

label = 'KOKX'
x2star,y2star = m(-72.865,40.865)
m.plot(x2star,y2star,'ro',markersize=12, color = 'k')
plt.text(x2star-0.4,y2star+0.15,label, weight = 'bold', size = 10)


latitude_path = [40.45,41.75,41.45,40.8,40.2,40,40.05,40.45]
longitude_path = [-73.85,-72.9,-71.85,-71.5,-72.2,-72.75,-73.3,-73.85]
...
plt.title('GPM Overpass 1/23/2016 Near Surface Precipitation Rate', size = 20)
...
plt.title('GPM Overpass 1/23/2016 LWP/IWP Ratio', size = 20)
...
plt.title('GPM Overpass 1/23/2016 IWP', size = 20)
...
plt.title('GPM Overpass 1/23/2016 Precipitation Category', size = 20)
...
plt.title('GPM Overpass 1/23/2016 Snowfall Rate (KuPR)', size = 20)
...
plt.title('GPM Overpass 1/23/2016 Snowfall Rate (KaPR)', size = 20)
